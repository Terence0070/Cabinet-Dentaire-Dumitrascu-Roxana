...
class Schedule {
    ...
    public function addSchedule($nom, $date_complet, $heure_debut, $heure_fin) {
        ...
        $query = "INSERT INTO horaires (nom, date_complet, heure_debut, heure_fin, modifie_par) VALUES (?, ?, ?, ?, ?)";
        if ($heure_debut >= $heure_fin) {
            return false;
        } else {
            $admin = $_SESSION['id'];
            $statement = $this->connexion->prepare($query);
            $statement->bind_param("sssss", $nom, $date_complet, $heure_debut, $heure_fin, $admin);
     ...
     */
    public function addMultipleSchedule($type, $heure_debut_lundi, $heure_debut_mardi, $heure_debut_mercredi, $heure_debut_jeudi, $heure_debut_vendredi, $heure_fin_lundi, $heure_fin_mardi, $heure_fin_mercredi, $heure_fin_jeudi, $heure_fin_vendredi, $date_debut, $date_fin) {
        // Vérifier si l'utilisateur a le rôle nécessaire
        if (!isset($_SESSION['idRole']) || $_SESSION['idRole'] != 1) {
            header('Location: index.php');
            exit();
        ...
        $query = "
        INSERT INTO horaires (nom, date_complet, heure_debut, heure_fin, modifie_par)
        ...
        // Binder les valeurs
        mysqli_stmt_bind_param($statement, "sssssssssssisss", 
            $type, 
            $heure_debut_lundi, $heure_debut_mardi, $heure_debut_mercredi, $heure_debut_jeudi, $heure_debut_vendredi, 
            $heure_fin_lundi, $heure_fin_mardi, $heure_fin_mercredi, $heure_fin_jeudi, $heure_fin_vendredi, 
            $admin, 
        ...
        if (mysqli_stmt_execute($statement)) {
            // Récupérer l'action de l'administrateur ainsi que son IP
            $action = "Ajout de plusieurs horaires réussis";
            $this->ipRecup($_SERVER['REMOTE_ADDR'], $action);
